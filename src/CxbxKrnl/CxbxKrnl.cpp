    ...
    void                  (*Entry)())
{
    ...
    if(DbgMode == DM_CONSOLE)
    {
        if(AllocConsole())
        {
			...
			printf("Please attach a debugger, then press Enter");
			getchar();
	...
	// Copy executable into modifiable ram
	g_XbeHeader = pXbeHeader;
	g_XbeBuffer = (char*)malloc(pXbeHeader->dwSizeofImage);
	memcpy(g_XbeBuffer, (void*)pXbeHeader->dwBaseAddr, pXbeHeader->dwSizeofImage);

	// Patch modifiable executable

	// Order changed to prevent FS Patching from breaking HLE detection
	...
	// Write new executable into memory space

	// The following code gets the process ID to default.exe and attaches to it with debug rights
	HANDLE hProcessSnap;
	HANDLE hProcess;
	PROCESSENTRY32 pe32;
	...
	// Loop through all processes until default.exe is found
	do
	{
		// TODO: Another method to detect Cxbx executable, since games with multiple XBE's will break here
		// If process is default.exe, open handle and break
		if (strcmp(pe32.szExeFile, "default.exe") == 0)
		{
	...
	// Now we have a handle to default.exe

	// Get Write Access
	VirtualProtectEx(hProcess, (LPVOID)pXbeHeader->dwBaseAddr, pXbeHeader->dwSizeofImage, PAGE_EXECUTE_READWRITE, NULL);
	
	// Write patched executable data
	WriteProcessMemory(hProcess, (LPVOID)pXbeHeader->dwBaseAddr, (void*)g_XbeBuffer, pXbeHeader->dwSizeofImage, NULL);
